...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (result) {
      ...
      res.status(404).json({ message: 'Pessoa não contrado' });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: err.sqlMessage })
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (result.affectedRows > 0) {
      res.status(200).json({ message: `Pessoa com ID:${id} atualizada` });
    } else {
      res.status(404).json({ message: 'Pessoa não contrado' });
    }
  } catch (error) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (result.affectedRows > 0) {
      res.status(200).json({ message: `Pessoa com ID:${id} excluída` });
    } else {
      res.status(404).json({ message: 'Pessoa não contrado' });
    }
  } catch (error) {
