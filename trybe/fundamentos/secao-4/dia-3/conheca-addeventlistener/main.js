...
const selectCard = (event) => {
  const select = document.querySelector(".tech");
  select.classList.remove("tech");
  event.target.classList.add("tech");
};

firstLi.addEventListener("click", selectCard);
secondLi.addEventListener("click", selectCard);
thirdLi.addEventListener("click", selectCard);
...
myWebpage.addEventListener("dblclick", () => {
  window.location.replace("https://kleber-ar.vercel.app/");
});
